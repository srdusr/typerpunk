...
impl Game {
    ...
    pub fn handle_backspace(&mut self, ctrl: bool) -> Result<bool, String> {
        ...
        if ctrl {
            ...
            // Same restriction plain Backspace enforces via
            // can_backspace_to_position, applied here too: only reach back
            // into an already-completed word when there's an actual
            // uncorrected mistake somewhere to fix. Otherwise clamp to the
            // start of the word currently being typed.
            let current_word_start = self.get_current_word_start();
            if word_start < current_word_start && !self.has_errors_before_position(chars.len()) {
                word_start = current_word_start;
            }
            if word_start >= chars.len() {
                return Ok(false);
            }
            new_input = chars[..word_start].iter().collect();
        } else {
    ...
    fn update_game_state(&mut self) {
        ...
        // Everything here counts CHARACTERS, never bytes. It used to mix the
        // two: `total_characters_typed` accumulated `self.input.len()` deltas
        // (a byte count) while `total_correct_characters` only incremented
        // when a char index reached `last_input_length` (that same byte
        // count). On ASCII the two are equal and it worked. On any text with
        // multi-byte characters the byte count outruns the char index, so the
        // correct-character counter never advanced and accuracy collapsed --
        // a Spanish passage reported 6% while every character matched. The
        // old byte slicing (`&self.input[..n]`) would also panic outright if
        // `n` ever landed inside a multi-byte character.
        ...
        let new_chars_typed = input_chars.len().saturating_sub(self.last_input_length);
        self.total_characters_typed += new_chars_typed as u32;

        let compared = input_chars.len().min(text_chars.len());
        for i in 0..compared {
            if input_chars[i] == text_chars[i] {
                current_streak += 1;
                best_streak = best_streak.max(current_streak);
                // Only positions newly reached this update count toward the
                // lifetime totals, or every keystroke would re-score the
        ...
        // Anything typed past the end of the passage is a mistake too.
        if input_chars.len() > text_chars.len() {
            current_mistakes += (input_chars.len() - text_chars.len()) as u32;
            if input_chars.len() > self.last_input_length {
                let prev_extra = self.last_input_length.saturating_sub(text_chars.len());
                let curr_extra = input_chars.len() - text_chars.len();
        ...
        self.last_input_length = input_chars.len();

        // Length-only: an uncorrected mistake anywhere in the input must
        // still let the test end once you've reached the end of the text,
        // exactly like every other typing-test engine (errors are scored via
        // accuracy, not something that blocks completion). Requiring an
        // exact match here meant a single uncorrected typo anywhere in the
        // text - not just the last character - made the test unfinishable:
        // the frontend caps input at text.len(), so once you hit that length
        // with any mismatch left in place, you can't type further and
        // is_finished() would never become true.
        let is_complete = !input_chars.is_empty() && input_chars.len() >= text_chars.len();
...
mod tests {
    ...
    // Regression: accuracy used to be computed by comparing a character index
    // against a byte-length counter, so any multi-byte character sent it to
    // near zero even when every keystroke was correct.
    #[test]
    fn accuracy_is_correct_for_multibyte_text() {
        let mut game = Game::new();
        game.set_text("años después".to_string());
        game.start();
        game.handle_input("años después").unwrap();
        assert_eq!(game.get_accuracy(), 100.0, "every character matched");
        assert_eq!(game.get_total_mistakes(), 0);
        assert!(game.is_finished(), "reaching the end must finish the test");
    }

    #[test]
    fn multibyte_mistakes_are_counted_by_character() {
        let mut game = Game::new();
        game.set_text("años".to_string());
        game.start();
        // Four characters typed, one of them wrong -> 75%, not a byte-derived
        // figure (the passage is five bytes long).
        game.handle_input("anos").unwrap();
        assert_eq!(game.get_accuracy(), 75.0);
    }
    ...
    fn test_backspace_to_previous_word_with_error() {
        let mut game = Game::new();
        // Longer than the typed input - typing exactly text.len() characters
        // finishes the game (see test_game_completion_detection), which
        // blocks backspace entirely. Mid-test, not-yet-complete input is also
        // the only way this scenario happens for a real user.
        game.set_text("Hello world today".to_string());
    ...
    fn test_ctrl_backspace() {
        // Longer than the typed input in each case below - see the comment
        // on test_backspace_to_previous_word_with_error for why.
        let mut game = Game::new();
        game.set_text("Hello world test today".to_string());
        ...
        // Test with no errors - should not allow ctrl+backspace to cross
        // into the previous (already-completed) word. Trailing space puts
        // the cursor right at the word boundary rather than mid-word --
        // mid-word, ctrl+backspace deleting the current word in progress is
        // always allowed regardless of errors, since that's not "reaching
        // back" into anything.
        let mut game2 = Game::new();
        game2.set_text("Hello world test today".to_string());
        ...
        game3.set_text("Hello world test today".to_string());
        ...
        // Should be able to ctrl+backspace again to go back further, since
        // the error in "Hallo" is still uncorrected - this was asserting
        // the same "Hallo world " value as the press above it (a copy-paste
        // leftover), which would mean this second press did nothing despite
        // reporting success.
        assert!(game3.handle_backspace(true).unwrap());
        assert_eq!(game3.get_input(), "Hallo ");
    }
    ...
    fn test_error_counting() {
        let mut game = Game::new();
        // Longer than every input below - reaching text.len() characters
        // finishes the game (see test_game_completion_detection), which
        // stops handle_input from processing anything further.
        game.set_text("Hello world today".to_string());
    ...
    fn test_ctrl_backspace_one_error_word_at_a_time() {
        let mut game = Game::new();
        // Longer than the typed input - see test_backspace_to_previous_word_with_error.
        game.set_text("foo bar baz qux end".to_string());
        game.handle_input("fao bar bzz qux").unwrap(); // errors in 'fao' and 'bzz'

        // First press deletes 'qux' - the word currently being typed, always
        // allowed regardless of errors since it's not "reaching back" into a
        // completed word.
        assert!(game.handle_backspace(true).unwrap());
        assert_eq!(game.get_input(), "fao bar bzz ");

        // Second press deletes 'bzz' - now the current word, same as above.
        assert!(game.handle_backspace(true).unwrap());
        assert_eq!(game.get_input(), "fao bar ");

        // Third press crosses from 'bar' back into 'fao' - allowed because
        // 'fao' has an uncorrected error.
        assert!(game.handle_backspace(true).unwrap());
        assert_eq!(game.get_input(), "fao ");
    }
}
